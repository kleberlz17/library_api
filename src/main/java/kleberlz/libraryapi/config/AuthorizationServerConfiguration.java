...
public class AuthorizationServerConfiguration {
	...
	@Bean // ABAIXO - CÓDIGO PARA CUSTOMIZAR AS INFORMAÇÕES DO TOKEN
	public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer(){
		return context -> {
			var principal = context.getPrincipal();
			
			if(principal instanceof CustomAuthentication authentication) { // VE SE É UMA  CUSTOM AUTHENTICATION
				OAuth2TokenType tipoToken = context.getTokenType();
				
				if(OAuth2TokenType.ACCESS_TOKEN.equals(tipoToken)) { // VÊ SE É UM ACCESS TOKEN
					Collection<GrantedAuthority> authorities = authentication.getAuthorities();  // ABAIXO PEGA AS AUTHORITIES 
